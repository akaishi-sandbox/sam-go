...
package:
	sam package --region ap-northeast-1 --s3-bucket unisize-artifacts-develop --s3-prefix toc-lambda --template-file ./template.yaml --output-template-file template-packaged.yml --profile develop

deploy:
	sam deploy --region ap-northeast-1 --template-file template-packaged.yml --stack-name toc-api --capabilities CAPABILITY_NAMED_IAM CAPABILITY_IAM --parameter-overrides $$(jq -r 'to_entries[] | "\(.key)=\(.value)"' ./prod.json)  --profile develop
